...
4. **F-016-1 (this re-walk) — the hosted mutation legs are not host-wired:** the web app's forms POST to `/flows/*` (e.g. `apps/web/src/pages/commerce-page.ts` line 92 `action: "/flows/place-order"`; the onboarding forms `apps/web/src/pages/onboarding-page.ts` lines 336, 405), and `apps/web/README.md` lines 137–147 assign the wiring of those form actions to the host ("a host … wires the rendered form actions (`/flows/*`, `data-flow` attributes) to the matching flow methods. The host owns sessions/CSRF"). The portal host implements the session/login POST, the page rendering, and the real `/v1` command plane — but has NO `/flows/*` handler (`apps/portal-host/src/app/[...surface]/route.ts` exports GET only; no flow handler exists in `apps/portal-host/src/handlers.ts`). On the live demo a browser user therefore cannot submit the rendered mutation forms; the journey mutations are proven through the app's typed flow methods over the real `/v1` mount (RL-113: `tests/e2e/test/hosted-commerce-support.test.ts` line 63). This is a standing never-wired state (present since RL-089 `fc8bec2`), unchanged by the PA wave — recorded here so a future work order can wire the form-action POSTs (or narrow the rendered forms) deliberately.
    - **F-016-1 closure note (PA-018, 2026-09-26, additive — the original finding above is unchanged):** the hosted form-action wiring landed. The portal host now serves the rendered `/flows/*` POSTs through the app's typed flow methods via a dedicated dispatcher (`apps/portal-host/src/flows.ts` + `handleFlowSubmit` in `apps/portal-host/src/handlers.ts`; the three-line Next.js forwarder at `apps/portal-host/src/app/flows/[name]/route.ts`). The wired action union is closed (17 actions): `enroll-device`, `update-device`, `retire-device`, `esim-install`, `esim-enable`, `esim-remove`, `mark-notification-read`, `create-intent`, `activate-intent`, `supersede-intent`, `create-support-case`, `place-order`, `record-payment`, `cancel-order`, `provision-connector`, `onboarding-enroll-device`, `onboarding-finish` — every rendered `data-flow` action across `apps/web/src/pages/**` is covered (the union is asserted by `tests/e2e/test/hosted-flows-form-actions.test.ts`'s "the host wires every rendered form action" row). Host-owned session/CSRF: the existing httpOnly `roamlink_session` cookie (SameSite=Lax) is the session surface; the seam's CSRF defense is a same-origin `Origin`/`Referer` check (no token in the rendered forms — the app never sees the CSRF surface, RL-LOCK-016). Response shape per page contract: most flows re-render the originating page with the typed `lastResult` panel above the body; the connector-enrollment redirect law (`/workspace?commandId=<ack.commandId>`) and the onboarding-finish law (redirect to `/`) fire on success. Fail-closed laws: runtime-not-ready → typed 503 `HOST_NOT_READY`; unknown flow → typed 404 `FLOW_NOT_FOUND`; CSRF failure → typed 403 `CSRF_INVALID`; session absent/unresolvable → 303 to `/login`; form-field missing/invalid → re-render with the typed `FORM_FIELDS_INVALID` panel (no command attempted); versioned-resource flows (update/retire device, activate/supersede intent, cancel order, onboarding-finish's activate leg) refuse the read-first leg under the uncomposed read model (F-016-2) and never issue a blind versionless write through the form plane. Battery evidence: `tests/e2e/test/hosted-flows-form-actions.test.ts` (32 tests, all green — the positive matrix across every wired action through the form-encoded path + the negative matrix + the closed-union law) and `apps/portal-host/test/flows.test.ts` (46 unit tests, all green — the dispatcher's parse/render/redirect laws pinned against the FLOW_HANDLERS table directly, including the provision-connector and onboarding-finish redirect laws). The wider `pnpm check` floor held (45 packages, 0 fail); no existing test was weakened, skipped or deleted. The F-016-2 read-model composition gap is unchanged (still open, not PA-018's scope); the form plane rides ABOVE the fail-closed page body exactly as the existing RL-113 e2e suite pins.
