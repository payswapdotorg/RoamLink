...
| `slo-journey-emission-matrix.test.ts` (RL-116) | the journey-state × §11-SLO emission matrix, EXECUTED: one row per §8 journey state + §10 failure state, driven through the real dogfood world with per-transition emission-delta assertions | presence AND absence are pinned — the reconciliation engine's durable actions mirror the emitted §11 events exactly (successful-automatic-recovery, closed stale/unknown windows, manual interventions), the pure intent-satisfaction mapping records good/bad outcomes at the product measurement point, and every HARNESS-only measurement point (time-to-usable, minutes-without, cost where available, failover, support incidents) is named as such; gap rows assert ZERO §11 emissions (no fabricated recovery while a stale/unknown window is open), so a regression that breaks or silently adds an emission path fails this suite |
